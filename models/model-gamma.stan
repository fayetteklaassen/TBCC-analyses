...
parameters {
  ...
  // real<lower=0,upper=1> prot_reinf;
  real<lower=0, upper =1> r_fast_slow;
  real<lower=0, upper =1> r_fast_sym;
  real<lower=0, upper =1> r_fast_clear;
  real<lower=0, upper =1> r_slow_clear;
  real<lower=0, upper =1> r_slow_sym;
  real<lower=0, upper =1> r_sym_die;
  real<lower=0, upper =1> r_sym_diag;
  real<lower=0, upper =1> r_sym_cure;
...
model {
  ...
  force ~ normal(0.4,.5);
// prot_reinf ~ beta(prot_reinf_lo, prot_reinf_hi);
r_fast_slow ~ gamma(r_fast_slow_lo, r_fast_slow_hi);
r_fast_sym ~ gamma(r_fast_sym_lo, r_fast_sym_hi);
r_fast_clear ~ gamma(r_fast_clear_lo, r_fast_clear_hi);
r_slow_clear ~ gamma(r_slow_clear_lo, r_slow_clear_hi);
r_slow_sym ~ gamma(r_slow_sym_lo, r_slow_sym_hi);
r_sym_die ~ gamma(r_sym_die_lo, r_sym_die_hi);
r_sym_diag ~ gamma(r_sym_diag_lo, r_sym_diag_hi);
r_sym_cure ~ gamma(r_sym_cure_lo, r_sym_cure_hi);
